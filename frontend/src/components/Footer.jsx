import { ArrowUp } from 'lucide-react';

const GithubIcon = ({ size = 16 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

const LinkedinIcon = ({ size = 16 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect x="2" y="9" width="4" height="12" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

export default function Footer({ site = {}, socials = [], nav = [] }) {
  const name = site.name || 'Gabrial Deora';
  const copyright = site.copyright || `© ${new Date().getFullYear()} ${name}. All Rights Reserved.`;

  const linkedin =
    site.linkedin ||
    (Array.isArray(socials) ? socials.find((s) => String(s.label || '').toLowerCase().includes('linkedin'))?.href : '');

  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <footer className="footer-modern-section">
      <div className="section-container">
        <div className="footer-main-row">
          <div>
            <div className="footer-brand-title">{name}</div>
            <div className="footer-brand-subtitle">Full Stack Web Developer &amp; Software Engineer</div>
          </div>

          <div className="footer-social-links">
            {site.github && (
              <a
                href={site.github}
                target="_blank"
                rel="noreferrer"
                className="footer-social-pill"
                data-cursor="GITHUB"
              >
                <GithubIcon size={15} />
                <span>GitHub</span>
              </a>
            )}
            {linkedin && (
              <a
                href={linkedin}
                target="_blank"
                rel="noreferrer"
                className="footer-social-pill"
                data-cursor="LINKEDIN"
              >
                <LinkedinIcon size={15} />
                <span>LinkedIn</span>
              </a>
            )}
            {site.email && (
              <a
                href={site.emailHref || `mailto:${site.email}`}
                className="footer-social-pill"
                data-cursor="MAIL"
              >
                <span>{site.email}</span>
              </a>
            )}
          </div>

          <button
            type="button"
            className="footer-back-to-top-btn"
            onClick={scrollToTop}
            data-cursor="TOP"
            aria-label="Back to Top"
          >
            <span>Back to top</span>
            <ArrowUp size={15} />
          </button>
        </div>

        {Array.isArray(nav) && nav.length > 0 && (
          <nav className="footer-nav-links" aria-label="Footer navigation">
            {nav.map((link) => (
              <a key={link.label || link.href} href={link.href} data-cursor="LINK">
                {link.label}
              </a>
            ))}
          </nav>
        )}

        <div className="footer-copyright-row">
          <span>{copyright}</span>
          <span>Crafted with precision &amp; modern web interactions</span>
        </div>
      </div>
    </footer>
  );
}
