  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-blue-50 via-white to-blue-100 shadow-lg border-b border-blue-100 backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
        <div className="text-2xl font-extrabold text-blue-700 tracking-tight cursor-pointer">
          {name}
        </div>
        {/* Desktop Nav */}
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden bg-white shadow-lg border-b border-blue-100 px-6 pb-4">
          <ul className="flex flex-col space-y-4 mt-2">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="flex items-center text-gray-700 font-semibold transition hover:text-blue-600 px-2 py-1"
